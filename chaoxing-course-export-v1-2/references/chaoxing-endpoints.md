...
1. Open the course `考试 -> 我的考试` page, usually:
   `/exam-ans/exam/test?classId=<classId>&courseId=<courseId>&ut=s&enc=<enc>&cpi=<cpi>&openc=<openc>`
2. Parse each completed exam item from `li.lookLi`.
3. The `查看` button usually stores the review URL in onclick:
   `location.href='/exam-ans/exam/test/reVersionPaperMarkContentNew?courseId=<courseId>&classId=<classId>&p=1&id=<exam-id>&ut=s&examsystem=0&cpi=<cpi>';`
4. Request each review page through the logged-in session.
5. Parse questions from `.TiMu`:
   - number/stem/score: `.Cy_TItle`
   - type: `input[name=type]` (`0` = 单选题, `3` = 判断题)
   - options: `.Cy_ulTop li`
   - correct and student answers: `.Py_answer span`
   - analysis: `.Py_addpyNr .pingyu`
6. Normalize 判断题 options and answers to `对`/`错`.
...
- Missing exam list: open the course exam page once in the controlled browser so `cpi`, `enc`, and `openc` are present.
