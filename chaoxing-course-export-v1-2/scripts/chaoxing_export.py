...
SOURCE_LABELS = {"quiz": "随堂练习", "homework": "作业", "exam": "考试"}
...
class CDP:
    ...
    def maybe_tab(self, pattern: str) -> dict[str, Any] | None:
        for tab in self.tabs():
            if pattern in tab.get("url", "") and tab.get("type") == "page":
                return tab
        return None
    ...
    async def exam_list_url(self) -> str | None:
        tab = self.maybe_tab("/exam-ans/exam/test")
        if tab:
            return tab.get("url")

        tab = self.maybe_tab("chaoxing.com")
        if not tab:
            return None
        async with websockets.connect(tab["webSocketDebuggerUrl"], max_size=80_000_000) as ws:
            await self.call(ws, "Runtime.enable")
                ...
                "Runtime.evaluate",
                {
                    "expression": """(() => {
                        const link = [...document.querySelectorAll('a')]
                          .find(a => (a.href || '').includes('/exam-ans/exam/test') || (a.getAttribute('data') || '').includes('/exam-ans/exam/test'));
                        if (!link) return '';
                        const raw = link.href && !link.href.startsWith('javascript') ? link.href : link.getAttribute('data');
            ...
            return result["result"]["result"].get("value") or None
...
def parse_exam_list_html(page_html: str, page_url: str) -> list[dict[str, str]]:
    soup = BeautifulSoup(page_html, "html.parser")
    exams = []
    for li in soup.select("li.lookLi"):
        title_el = li.select_one(".titTxt a") or li.select_one("a")
        title = clean_text(title_el.get_text(" ", strip=True)) if title_el else ""
        if not title:
            title = clean_text(li.get_text(" ", strip=True)).split("考试时间：")[0].strip()

        view = li.select_one("a.Btn_blue_1") or next((a for a in li.select("a") if "查看" in clean_text(a.get_text(" ", strip=True))), None)
        if not view:
            continue
        onclick = view.get("onclick") or ""
        match = re.search(r"location\.href='([^']+)'", onclick)
        url = html.unescape(match.group(1)) if match else html.unescape(view.get("href") or "")
        ...
        status = ""
        if "考试时间：" in text:
            time_value = text.split("考试时间：", 1)[1].split("考试状态：", 1)[0].strip()
        if "考试状态：" in text:
            status = text.split("考试状态：", 1)[1].strip()
        oper = clean_text(li.select_one(".titOper").get_text(" ", strip=True)) if li.select_one(".titOper") else ""
        score_match = re.search(r"(\d+(?:\.\d+)?)\s*分", oper)
        score = score_match.group(1) if score_match else ""
        if score and f"{score}分" in status:
            status = status.split(f"{score}分", 1)[0].strip()
        if "查看" in status:
            status = status.split("查看", 1)[0].strip()
        exam_id = query(url).get("id", "")
        exams.append({"title": title, "time": time_value, "status": status, "score": score, "exam_id": exam_id, "url": url})
    return exams


def parse_exam_list(session: requests.Session, exam_list_url: str) -> list[dict[str, str]]:
    response = session.get(exam_list_url, timeout=40)
    response.raise_for_status()
    return parse_exam_list_html(response.text, response.url)


def clean_exam_answer(value: Any, qtype: Any) -> str:
    text = clean_text(value)
    if "：" in text:
        text = text.split("：", 1)[1]
    elif ":" in text:
        text = text.split(":", 1)[1]
    return normalize_answer_for_type(text, qtype)


def strip_exam_score_suffix(stem: str, score: str) -> str:
    stem = clean_text(stem).replace("( )", "（ ）")
    suffixes = []
    if score:
        suffixes.extend([f"（{score}分）", f"({score}分)"])
...
def parse_exam_question(node, exam_title: str, order: int) -> dict[str, Any]:
    title_node = node.select_one(".Cy_TItle div.fl.clearfix")
    num_node = node.select_one(".Cy_TItle i.fl")
    type_input = node.select_one('input[name="type"]')
    score_input = node.select_one('input[name="score"]')
    qtype = normalize_question_type((type_input.get("value") if type_input else "") or "")
    num = clean_text(num_node.get_text(" ", strip=True)) if num_node else str(order)
    score = score_input.get("value") if score_input else ""
    if not score:
        score_node = node.select_one(".Cy_TItle span.fr i")
        score = clean_text(score_node.get_text(" ", strip=True)) if score_node else ""
    stem = strip_exam_score_suffix(title_node.decode_contents() if title_node else "", score)

    options = []
    for li in node.select(".Cy_ulTop li"):
        label_el = li.select_one("i.fl")
        label = clean_text(label_el.get_text(" ", strip=True)) if label_el else ""
        label = re.sub(r"[^A-Ea-e]", "", label).upper()
        content = clean_text(" ".join(clean_text(child.get_text(" ", strip=True)) for child in li.select("div.clearfix")) or li.get_text(" ", strip=True))
        if label:
            content = re.sub(rf"^{re.escape(label)}\s*[、.，]\s*", "", content, count=1)
        if not is_true_false_type(qtype) and (label or content):
    ...
    correct = ""
    student_answer = ""
    answer_box = node.select_one(".Py_answer")
    if answer_box:
        spans = answer_box.select("span")
        if spans:
            correct = clean_exam_answer(spans[0].get_text(" ", strip=True), qtype)
        if len(spans) > 1:
            student_answer = clean_exam_answer(spans[1].get_text(" ", strip=True), qtype)
    if not student_answer:
        my_input = node.select_one('input[name="myanswer"]') or node.select_one("#myanswer")
        if my_input:
            student_answer = normalize_answer_for_type(my_input.get("value", ""), qtype)

    analysis_node = node.select_one(".Py_addpyNr .pingyu")
    qid_input = node.select_one('input[name="qid"]')
    return {
        "source": "exam",
        "set": exam_title,
        "num": num,
        ...
        "answer": correct,
        "student_answer": student_answer,
        "analysis": clean_text(analysis_node.get_text(" ", strip=True)) if analysis_node else "",
        "answer_source": "chaoxing" if correct else "unpublished",
        "qid": qid_input.get("value") if qid_input else "",
    }


def parse_exam_detail_html(page_html: str, exam: dict[str, Any]) -> dict[str, Any]:
    soup = BeautifulSoup(page_html, "html.parser")
    questions = [parse_exam_question(node, exam.get("title", "考试"), idx) for idx, node in enumerate(soup.select(".TiMu"), 1)]
    text = clean_text(soup.get_text(" ", strip=True))
    detail_score = ""
    if "成绩：" in text:
        detail_score = text.split("成绩：", 1)[1].split("分", 1)[0].strip()
        if detail_score:
            detail_score += "分"
    return {**exam, "detail_score": detail_score, "question_count": len(questions), "questions": questions}


def parse_exam_detail(session: requests.Session, exam: dict[str, Any]) -> dict[str, Any]:
    response = session.get(exam["url"], timeout=50)
    response.raise_for_status()
    return parse_exam_detail_html(response.text, exam)


def flatten_exam_questions(exams: list[dict[str, Any]]) -> list[dict[str, Any]]:
    questions = []
    for exam in exams:
        for question in exam.get("questions") or []:
            item = dict(question)
            item["options"] = [f"{option['label']}. {option['text']}" if option.get("label") else option.get("text", "") for option in item.get("options") or []]
            questions.append(item)
    return questions


def export_exams(session: requests.Session, out: Path, exam_list_url: str | None) -> list[dict[str, Any]]:
    if not exam_list_url:
        return []
    exams = [parse_exam_detail(session, exam) for exam in parse_exam_list(session, exam_list_url)]
    flat = flatten_exam_questions(exams)
    (out / "chaoxing_exam_questions_raw.json").write_text(json.dumps({"exam_count": len(exams), "total_questions": len(flat), "exams": exams}, ensure_ascii=False, indent=2), encoding="utf-8")
    build_docx(out / "chaoxing_exam_questions.docx", "学习通考试题目", flat, include_answers=False)
    build_docx(out / "chaoxing_exam_answers.docx", "学习通考试答案", flat, include_answers=True)
    return exams
...
async def main() -> None:
    ...
    parser.add_argument("--mode", choices=["questions", "courseware", "exams", "all"], default="all")
    ...
    exams = []
    if args.mode in {"exams", "all"}:
        exams = export_exams(session, out, await cdp.exam_list_url())

    exam_questions = sum(item.get("question_count", 0) for item in exams)
    outputs = ["chaoxing_export_report.json"]
    if questions:
        outputs.extend(["chaoxing_questions_raw.json", "chaoxing_review_questions.docx", "chaoxing_review_answers.docx"])
    if exams:
        outputs.extend(["chaoxing_exam_questions_raw.json", "chaoxing_exam_questions.docx", "chaoxing_exam_answers.docx"])
    if courseware:
        outputs.extend(["chaoxing_courseware_manifest.json", "courseware/"])
    report = {
        ...
        "total_exam_questions": exam_questions,
        "exam_with_answers": sum(1 for exam in exams for item in exam.get("questions", []) if item.get("answer")),
