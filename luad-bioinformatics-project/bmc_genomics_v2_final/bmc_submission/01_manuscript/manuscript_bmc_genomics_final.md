# A literature-guided public-data evidence hierarchy prioritises a PLK1-associated mitotic and ER-redox signature in lung adenocarcinoma
...
| --- | --- | ---: | --- | --- |
| TCGA-LUAD | Bulk RNA-seq, survival, clinical, mutation | 503 primary-tumour patients (patient-level) | Discovery, training-only screen, held-out test | Discovery/internal; exploratory, not validated |
...
- Figure 2. Patient-level rebuild: no genome-wide-FDR-significant gene on training-only screening; a nominal signature gives modest held-out concordance (0.62) and weak, CI-wide external transfer.
- Figure 3. Pathway enrichment and Human Protein Atlas biological-context coherence.
- Additional file (Supplementary). CELLxGENE aggregate and E-MTAB-13530 Visium/spatial-proxy tissue context (the aggregate screens that motivated the single-cell and deconvolution analyses in Figures 6-7); moved out of the main figures to avoid duplication.
- Figure 5. DepMap PLK1 dependency, pharmacogenomic boundaries and negative ERO1A-modulation/lineage-selectivity evidence.
- Figure 6. Single-cell (GSE131907): patient-level (paired, 10 patients) enrichment of the signature in tumour-derived epithelium (7/9 genes FDR<0.05), cell-type localisation, malignant cell-state peak and proliferation link.
