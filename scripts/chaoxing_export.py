...
鏈叕甯?and never attempts to submit or modify coursework.
"""
...
def parse_homework_list(session: requests.Session, course_id: str, class_id: str, cpi: str) -> list[dict[str, str]]:
    ...
    for li in soup.select('li[onclick*="goTask"]'):
        ...
        works.append({"source": "浣滀笟", "name": name, "status": clean_text(status_el.get_text(" ")) if status_el else "", "url": task_url})
...
def parse_homework_page(session: requests.Session, work: dict[str, str], course_id: str, class_id: str, cpi: str) -> list[dict[str, Any]]:
    ...
    for q in soup.select(".questionLi"):
        ...
        match = re.match(r"^(\d+)\.\s*(?:\(([^)锛塢+)[)锛塢)?\s*(.*)$", raw, flags=re.S)
    ...
    for q in questions:
        q["answer_source"] = "瀛︿範閫? if q["answer"] else "鏈叕甯?
    return questions
...
def parse_activity_list(session: requests.Session, fid: str, course_id: str, class_id: str) -> list[dict[str, str]]:
    ...
    return [
        {"source": "闅忓爞缁冧範", "name": a.get("nameOne") or f"闅忓爞缁冧範 {a.get('id')}", "status": str(a.get("status")), "id": str(a.get("id"))}
        for a in data
        ...
        questions.append(
            {
                "source": "闅忓爞缁冧範",
                "set": title,
                "status": "杩涜涓? if activity.get("status") == "1" else "宸茬粨鏉?,
                "num": str(idx),
                "type": TYPE_MAP.get(q.get("type"), f"棰樺瀷{q.get('type')}"),
                ...
                "answer_source": "瀛︿範閫? if answer else "鏈叕甯?,
            }
        )
    return questions
...
def build_docx(path: Path, title: str, questions: list[dict[str, Any]], include_answers: bool) -> None:
    ...
    for q in questions:
        ...
        if include_answers:
            body.append(para(f"绛旀锛歿q.get('answer') or '鏈叕甯?}锛堟潵婧愶細{q.get('answer_source') or '鏈叕甯?}锛?))
            if q.get("analysis"):
                body.append(para(f"瑙ｆ瀽锛歿q['analysis']}"))
            if q.get("student_answer"):
                body.append(para(f"鎴戠殑绛旀锛歿q['student_answer']}"))
...
async def main() -> None:
    ...
    build_docx(out / "chaoxing_review_questions.docx", "瀛︿範閫氭湡鏈涔犻", questions, include_answers=False)
    build_docx(out / "chaoxing_review_answers.docx", "瀛︿範閫氭湡鏈涔犻绛旀", questions, include_answers=True)
    report = {
