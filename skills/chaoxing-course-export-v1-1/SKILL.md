---
name: chaoxing-course-export-v1-1
description: Export authorized Chaoxing/Xuexitong course materials for review, version 1.1. Use when Codex needs to download PPT/PDF courseware from chapter pages, collect completed or visible homework and in-class practice questions, handle updated homework iframes carrying stuenc/enc parameters, extract published answers, and generate local review DOCX/JSON files from a logged-in browser session.
---

# Chaoxing Course Export v1.1

Use this skill to help a user archive their own Chaoxing/Xuexitong course resources into local study files.

This is a separate v1.1 skill and should not overwrite the original `chaoxing-course-export` skill.
...
- For `判断题`, normalize options and answers to `对`/`错`; do not leave them as `1`/`2` or `A`/`B`.
